...
import 'package:log_table/ui/providers/inverse_view_model_provider.dart';
import 'package:syncfusion_flutter_datagrid/datagrid.dart';

import 'log_data_source.dart';

class InversePage extends ConsumerWidget {
  const InversePage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final theme = Theme.of(context);
    final scheme = theme.colorScheme;
    final model = ref.watch(inverseViewModelProvider);
    return Scaffold(
      appBar: AppBar(
        title: const Text("INVERSE"),
        actions: [
          Column(
            ...
            (index) {
              ...
              return [
                  ...
                  List.generate(
                    10,
                    (rowIndex) {
                      final Log log = Inverse.logCell(number, rowIndex);
                      return LogData(
                          label: log.label,
                          value: log.value,
                          rowSelected: model.rowIndex == number,
                  ...
                  List.generate(
                    9,
                    (rowIndex) {
                      final Mean log = Inverse.meanCell(number, rowIndex + 1);
                      return LogData(
                        label: log.label,
                        value: log.value,
                        rowSelected: model.rowIndex == number,
