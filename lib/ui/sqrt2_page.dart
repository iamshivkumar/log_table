...
import 'providers/sqrt2_view_model_provider.dart';

class Sqrt2Page extends ConsumerWidget {
  const Sqrt2Page({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final model = ref.watch(sqrt2ViewModelProvider);
    return Scaffold(
      appBar: AppBar(
            ...
            (index) {
              ...
              return [
                  ...
                  List.generate(
                    10,
                    (rowIndex) {
                      final Log log = Sqrt2.logCell(number, rowIndex);
                      return LogData(
                          label: log.label,
                          value: log.value,
                          rowSelected: model.rowIndex == number,
                  ...
                  List.generate(
                    9,
                    (rowIndex) {
                      final Mean log = Sqrt2.meanCell(number, rowIndex + 1);
                      return LogData(
                        label: log.label,
                        value: log.value,
                        rowSelected: model.rowIndex == number,
