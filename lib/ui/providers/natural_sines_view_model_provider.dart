...
final naturalSinesModelProvider = ChangeNotifierProvider(
...
class NaturalSinesViewModel extends ChangeNotifier {
  ...
  String get value {
    if (_rowIndex == null || _logIndex == null) {
      ...
      final double log =
          NaturalSines.sinCell(_rowIndex!, _logIndex! / 10).value;
      if (_meanIndex != null) {
        final String q = "${_rowIndex}°${_logIndex! * 6 + _meanIndex!}'";
        final double mean =
            NaturalSines.meanCell(_rowIndex!, _meanIndex!).value;
        return "sin($q) = " + 
